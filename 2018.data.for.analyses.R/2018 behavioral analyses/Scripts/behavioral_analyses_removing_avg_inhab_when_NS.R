# Description: analyses for goby behavioral observations
# Author: George C Jarvis
# Date: Tue Oct 26 14:12:53 2021
# Notes: mixed models typically had singular fits, so purging all models where models are singular
#       - interactions between fixed effects and trial generally resulted in singular model fits   
#       - wanted to keep random effect of trial in there though when possible
# --------------
...
# full model with all possible interactions between fixed and random effects (fit is singular, so removed all random effects, except for trial)
...
# removing avg.inhab (NS), putting treatment*year back into the model

pe.1d_dra<-lmer(proportion.exposed~ Treatment + Year + Treatment*Year +
                 (1|Trial), data = behave)
summary(pe.1d_dra) # trial explain 3% of the residual variance, low, but think it's worth including
Anova(pe.1d_dra, type = "III")

Anova(pe.1d_dra)

# MS - final results for proportion of time exposed
...
# Response: proportion.exposed
#                  Chisq Df Pr(>Chisq)    
# (Intercept)      885.4199  1     <2e-16 ***
# Treatment        8.7015  2     0.0129 *  
# Year             1.9309  1     0.1647    
# Treatment:Year   0.4133  2     0.8133    
...
emmeans(pe.1d_dra, pairwise ~ Treatment) # these aren't that different from the original means +/- se, going to leave them
# if I end up having to go back and replot some behaviors, then I will change these as well

# means and se for proportion of time exposed, note that these are based on predictions from mod: pe.1d
...
# note that there is a sig. effect of avg inhab - gobies traveled further when more gobies were on the reef

# plotted adjusted means
...
# MS - final model results linear distance travelled (mixed model) ######

dm.1e_remlF_a<-lmer(total.dist.moved~ Treatment + Year + avg.inhab + Treatment*Year +
                    (1|Trial), REML=F, data = behave)
Anova(dm.1e_remlF_a, type = "III")
...
# significant treatment effect when I include the treatment*year effect


# looking at summary to see if therer is a change in sign in the treatment effect +/- treatment*year effect

summary(dm.1e_remlF_a)

# Fixed effects:
#   Estimate Std. Error t value
# (Intercept)      -21.3380    58.7765  -0.363
# Treatment1       -11.2986    10.1694  -1.111 # interested in these coefficients
# Treatment2        25.5114    10.1699   2.509 # here too
# Year1             -6.9760    20.3958  -0.342
# avg.inhab         12.6600     4.2717   2.964
# Treatment1:Year1  -0.6815    10.2542  -0.066
# Treatment2:Year1 -14.2164    10.1564  -1.400

# now removing interaction

dm.1e_remlF_b<-lmer(total.dist.moved~ Treatment + Year + avg.inhab +
                      (1|Trial), REML=F, data = behave)
Anova(dm.1e_remlF_b, type = "III")
...
summary(dm.1e_remlF_b) # coefficients are nearly identical, keeping the interaction term in there is okay, just soaks up some residual variance

# Fixed effects:
#   Estimate Std. Error t value
# (Intercept)  -16.256     59.111  -0.275
# Treatment1   -11.490     10.163  -1.131
# Treatment2    23.109     10.176   2.271
# Year1         -6.619     20.885  -0.317
# avg.inhab     12.229      4.283   2.855

# What does emmeans show?

emmeans(dm.1e_remlF_a, pairwise~Treatment)

# > emmeans(dm.1e_remlF_a, pairwise~Treatment)
# NOTE: Results may be misleading due to involvement in interactions
# $emmeans
# Treatment emmean   SE   df lower.CL upper.CL
# High         131 28.3 16.8     71.4      191
# Low          168 28.3 16.8    108.2      228
# Medium       128 28.1 16.4     68.8      188
# 
# Results are averaged over the levels of: Year 
# Degrees-of-freedom method: kenward-roger 
# Confidence level used: 0.95 
...
# $contrasts
# contrast      estimate   SE   df t.ratio p.value
# High - Low      -36.81 18.1 91.3 -2.039  0.1087 
# High - Medium     2.91 18.2 91.8  0.160  0.9860 
# Low - Medium     39.72 18.2 91.8  2.182  0.0796 
# 
# Results are averaged over the levels of: Year 
# Degrees-of-freedom method: kenward-roger 
# P value adjustment: tukey method for comparing a family of 3 estimates 

# relationship looks slightly different, going to plot this out and see what it looks like

dm.1e.emm.df <- as.data.frame(emmeans(dm.1e_remlF_a, pairwise~Treatment))
...
# doesn't look different enough for me to change the figure, it's okay

# 1c. foraging rate ####

# model fit is singluar if we include trial as an effect, running as linear model
...
# going to rerun this, without avg.inhab, and with treatment*year effect

# MS - Final model, foraging rate ####

# fit is singluar, so running as general linear model

fr_fix5 <-lm(bites.min ~ Treatment + Year + Treatment*Year, data = behave) # no treatment effect when T*Y removed
Anova(fr_fix5, type = "III")
...
# Response: bites.min
#                 Sum Sq Df  F value Pr(>F)    
# (Intercept)    43.703  1 158.2151 <2e-16 ***
# Treatment       0.601  2   1.0876 0.3416    
# Year            0.006  1   0.0217 0.8833    
# Treatment:Year  0.632  2   1.1443 0.3232    
# Residuals      24.032 87
...
emmeans(fr_fix5, pairwise~Treatment) # again, not big enough differences to change the entire plot, same qualitative effects
...
# removing avg.inhab, putting treatment*year back into the model

ci.a5 <-lmer(courtship.min ~ Treatment + Year + (Treatment*Year) +
               (1|Trial), REML=F, data = behave)
Anova(ci.a5, type = "III")

# MS - results for conspefici interaction rate
...
# Response: courtship.min
# Chisq Df Pr(>Chisq)    
# (Intercept)    95.4206  1     <2e-16 ***
#   Treatment       3.0537  2     0.2172    
# Year            0.6373  1     0.4247    
# Treatment:Year  1.0417  2     0.5940    

# removing trt*year to see if that affects other factors

ci.a5a <-lmer(courtship.min ~ Treatment + Year +
               (1|Trial), REML=F, data = behave)
Anova(ci.a5a, type = "III") # nah, no difference, keeping the treatment*year term in the model
...
# removing avg.inhab, putting trt*year back into the model, also, can run this as a mixed model, so doing that to account
# - for trial effects

mr.a_f5 <-lmer(movements.min ~ Treatment + Year + Treatment*Year + (1|Trial), data = behave)
summary(mr.a_f5) # again, minimal variance explained by trial, but keeping in anyway

Anova(mr.a_f5, type = "III")

# MS - final results, movement rate (was able to run as mixed model) #####
...
# Response: movements.min
# Chisq Df Pr(>Chisq)    
# (Intercept)    442.6749  1     <2e-16 ***
#   Treatment        3.8300  2     0.1473    
# Year             0.5758  1     0.4480    
# Treatment:Year   0.3193  2     0.8524   

mr.a_f4.emm <- emmeans(mr.a_f4, ~ Treatment)

mr.a_f4.emm <- emmeans(mr.a_f5, ~ Treatment) # looks very similar to what I plotted, no need to change
